...
def download_oxides_from_matproj(MAPI_KEY, elements_to_avoid, file_name):
    ...
    data = mpr.summary.search(elements = ['O'], 
                              num_elements = [2,3],
    ...
    for ii in range(0,len(data)):
      ...
      elif np.array_equal(np.sort(stoich),[1, 1]) is True:
    ...
    for ii in range(0, len(elements_to_avoid)):
      df = df[df.elements.map(set([elements_to_avoid[ii]]).issubset) == False]
